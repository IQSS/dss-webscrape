...
Let us look at an example dynamic web page, which is available at [Dynamic Search](http://127.0.0.1:8000/tutorial/cases/search). This example website has a search form that is used to locate students. Let us say we want to find all the students whose name begins with the letter A and who are in the fifth grade:
...
search_url = "http://127.0.0.1:8000/tutorial/cases/search"
...
Look at the two links highlighted in **Figure I-1.2**. These two links have a tag name “**a**” with an attribute “**href**”. The first link is a static link, while the second is a dynamic link. Normally, if a link is a static link, its link address will end with a file name with a file extension. In this case, the link address (*http://127.0.0.1:8000/tutorial/static/views/Adams.html*) ends with a **.html** file. The link address is formed first starting with a local server address (*http://127.0.0.1:8000*) and followed by the web application name (*/tutorial*) and then followed by a hierarchical path of folders to that **.html** file. 

| **Figure I-2.1** |
|:--:|
| ![](images/I-2/1.png) |
On the other hand, if a link is a dynamic link, you will not find a file extension at the end of its link address. For example, in this case, the link address (*http://127.0.0.1:8000/tutorial/cases/getstudent/Adams*) has no a file extension at the end. It is formed first beginning at a local server address (*http://127.0.0.1:8000*) and followed by the web application name (*/tutorial*) and then followed by the name of the Python script (*cases.py*) in the web application (*/cases*) that calls a function in this script called as “**getstudent()**” (*/getstudent*) followed with a parameter value (*/Adams*) to the **getstudent()** function.

| **Figure I-2.2** |
|:--:|
| ![](images/I-2/2.png) |  
Suppose that we temporarily disable the JavaScript execution functionality in our browser by changing the browser’s settings. Let us copy the static link address and open it in a new browser. We should be able to see the profile of the chosen student as **Figure 1-2.3** shows. But if we copy the dynamic link address and open it in a new browser, we will only see an empty webpage with just page layout as **Figure 1-2.4** shows. This is because the new browser whose JavaScript functionality has been disabled cannot execute JavaScript code in order to display the profile content.  
...
```{python, eval=TRUE}
staticLink_url = "http://127.0.0.1:8000/tutorial/static/views/Adams.html"
...
dynamicLink_url = "http://127.0.0.1:8000/tutorial/cases/getstudent/Adams"
...
print(page_link)
```
**Figure I-3.4** contrasts the result table part of code in the original HTML from the **page source code** (left figure) versus the revised HTML from the **Elements** window after it executes the JavaScript code (right figure). It is clear to see that there is no information under the tag name “**tbody**” on the left. This explains why the xpath() method returns an empty list. On the right, the webpage runs the JavaScript to insert the first chunk of the students’ information into the empty result table that has been created statically before running the JavaScript and then display it in the browser.  

| **Figure I-3.4** |
|:--:|
| ![](images/I-3/6.png) |
More interestingly, once we open the **Elements** window of this example webpage, under the “**tbody**” tag, we find there are eight table entries (with tag "**tr**"). We can therefore infer that a load has a total of eight table entries. If we scroll down to the bottom of the webpage, we could see that three more entries are appended to the table. If we continue to scroll down to the bottom of the page, nothing is changed. This means that there exists a total of eleven table entries and the new load loads the next three new ones after the first batch.  Your browser executes the JavaScript code to perform all these actions. If you look at the JavaScript section in the **page source code**, you will see that a JQuery method “**\$(window).on("scroll", function()**” defines the scrolling down to the bottom of page and triggers the load of the new content once it is satisfied. And another JQuery method “**\$('#resultstable > tbody:last-child').append(htmlContent)**” append the new entries to the result table. Here the screenshot of the relevant parts of the JavaScript code is not shown. 